...
const GameLobbyPage = (props: GameLobbyPageProps) => {
  ...
  const fetchPlayers = useCallback(async () => {
    ...
    if (playersResponse.status === 200) {
      const newPlayers = deserializeToInstances<Player>(
        Player,
        ...
        for (const message of deserializedData) {
          ...
          if (message?.instance?.class === "game") {
            ...
            if (message?.operation === "update") {
              const updatedGame = deserializeToInstance<Game>(
                Game,
          ...
          if (message?.instance?.class === "player") {
            if (message?.operation === "create") {
              // Add a game player
              const newPlayer = deserializeToInstance<Player>(
                Player,
                message.instance.data
              )
              ...
              if (
                newPlayer &&
                !playersRef.current.some((p) => p.id === newPlayer.id)
              ) {
              ...
              const idToRemove = message.instance.id
              setPlayers((players) =>
                players.filter((p) => p.id !== idToRemove)
              )
            }
          }
          ...
          if (message?.instance?.class === "turn") {
            ...
            if (message?.operation === "create") {
              const updatedTurn = deserializeToInstance<Turn>(
                Turn,
                message.instance.data
              )
              if (updatedTurn) {
          ...
          if (message?.instance?.class === "phase") {
            ...
            if (message?.operation === "create") {
              const updatedPhase = deserializeToInstance<Phase>(
                Phase,
                message.instance.data
              )
              if (updatedPhase) {
          ...
          if (message?.instance?.class === "step") {
            ...
            if (message?.operation === "create") {
              const updatedStep = deserializeToInstance<Step>(
                Step,
                message.instance.data
              )
              if (updatedStep) {
  ...
  const handleDelete = () => {
    const deleteGame = async () => {
      if (
        window.confirm(`Are you sure you want to permanently delete this game?`)
      ) {
        const response = await request(
          "DELETE",
          `games/${props.gameId}/`,
  ...
  const overview = [{ key: "Host", value: game.host?.username ?? "" }]
  if (!loading) {
    if (latestTurn && latestPhase) {
      overview.push({ key: "Start Date", value: getFormattedStartDate() })
      overview.push({
        key: "Progress",
        value: `Turn ${latestTurn.index}, ${latestPhase.name} Phase`,
      })
    } else {
      overview.push({
        key: "Creation Date",
        value: formatDate(game.creation_date, props.clientTimezone),
      })
  ...
  return (
    <>
      <Head>
        <title>
          {game
            ? `${game.name} (Lobby) | Republic of Rome Online`
      ...
      <main className="standard-page">
        ...
        <h2 id="page-title" style={{ marginBottom: 0 }}>
          {game.name}
        </h2>
        <h3 style={{ marginTop: 0, color: "var(--foreground-color-muted)" }}>
          Game Lobby
        </h3>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} gap={{ xs: 2 }}>
            <Card sx={{ width: { xs: "100%", sm: "50%" } }}>
              <Card variant="outlined" style={{ height: "100%" }}>
                <h3 style={{ marginLeft: "16px", marginBottom: 0 }}>
                  Overview
                </h3>
                  ...
                  <>
                            ...
                            return (
                              ...
                              >
                                <ListItemAvatar>
                                  <Avatar>
                                    {capitalize(
                                      player.user.username.substring(0, 1)
                                    )}
                                  </Avatar>
                                </ListItemAvatar>
                                <ListItemText>
                    ...
                    {players.length < 3 && (
                      <div style={{ marginLeft: 16, marginBottom: 6 }}>
                        <p style={{ margin: 0 }}>
                          You need at least {6 - players.length - 3} more to
                          start
                        </p>
                      </div>
                    )}
            ...
            <Card>
              <Card variant="outlined" style={{ padding: "16px" }}>
                ...
                <Stack spacing={2} direction={{ xs: "column", sm: "row" }}>
                  ...
                  {!players.some(
                    (p) => p.user instanceof User && p.user.id === user.id
                  ) &&
                    players.length < 6 &&
                    !latestStep && (
