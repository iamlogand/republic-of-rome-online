...
def test_land_bill_repeal_pass_removes_effect_marker(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
...
@pytest.mark.django_db
def test_land_bill_repeal_pass_increases_unrest(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    game.unrest = 1
    game.add_effect(GameEffect.LAND_BILL_2)
...
@pytest.mark.django_db
def test_land_bill_repeal_pass_reduces_sponsor_popularity(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    game.add_effect(GameEffect.LAND_BILL_2)
    game.save()
    ...
    assert sponsor.popularity == 2  # 5 - 3 (sponsor -2 + voting -1)


@pytest.mark.django_db
def test_land_bill_repeal_pass_type_iii_sponsor_popularity(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    ...
    # Assert
    sponsor.refresh_from_db()
    assert sponsor.popularity == 2  # 8 - 6 (sponsor -4 + voting -2)
...
def test_land_bill_repeal_pass_voted_yea_non_sponsor_loses_popularity(
    ...
    sponsor = senators[0]
    other = senators[1]
    game.current_proposal = f"Repeal type II land bill sponsored by {sponsor.display_name}"
    game.votes_yea = 15
    game.votes_nay = 0
    game.save()
...
@pytest.mark.django_db
def test_land_bill_repeal_pass_only_removes_one_marker(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    game.add_effect(GameEffect.LAND_BILL_2)
    game.add_effect(GameEffect.LAND_BILL_2)
...
@pytest.mark.django_db
def test_land_bill_repeal_pass_blocks_second_repeal(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    game.add_effect(GameEffect.LAND_BILL_2)
    game.save()
...
@pytest.mark.django_db
def test_land_bill_repeal_fail_blocks_second_repeal(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    game.add_effect(GameEffect.LAND_BILL_2)
    game.save()
...
@pytest.mark.django_db
def test_land_bill_repeal_fail_keeps_effect_marker(senate_game: Game, resolver: FakeRandomResolver):
    # Arrange
    game = senate_game
    game.add_effect(GameEffect.LAND_BILL_2)
    game.save()
