...
def test_land_battle_victory(basic_game: Game):
...
@pytest.mark.django_db
def test_land_battle_victory_2_losses(basic_game: Game):
    ...
    # Assert
    surviving_legion_count = Legion.objects.filter(game=game).count()
    assert surviving_legion_count == 8
    game.refresh_from_db()
    assert game.unrest == 2
    commander.refresh_from_db()
    assert commander.alive == True
    assert commander.popularity == 4
    assert commander.location == "Rome"
    assert Campaign.objects.filter(game=game).exists() == False
    

@pytest.mark.django_db
def test_land_battle_stalemate(basic_game: Game):
...
@pytest.mark.django_db
def test_land_battle_stalemate_2_losses(basic_game: Game):
...
@pytest.mark.django_db
def test_land_battle_defeat_1_loss(basic_game: Game):
    ...
    # Assert
    surviving_legion_count = Legion.objects.filter(game=game).count()
    assert surviving_legion_count == 9
    game.refresh_from_db()
...
@pytest.mark.django_db
def test_land_battle_defeat_5_losses(basic_game: Game):
...
@pytest.mark.django_db
def test_land_battle_defeat_total(basic_game: Game):
