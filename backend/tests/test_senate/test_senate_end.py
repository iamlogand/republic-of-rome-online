...
def test_commanderless_campaign_recalled_at_senate_close(basic_game: Game):
    """A campaign with no commander should be automatically recalled at senate close."""
    ...
    # Assert: campaign was deleted
    assert Campaign.objects.filter(game=game).count() == 0
    # Legions returned to reserve (campaign SET_NULL → campaign=None)
    assert Legion.objects.filter(game=game, campaign__isnull=False).count() == 0
    assert Legion.objects.filter(game=game).count() == 4
