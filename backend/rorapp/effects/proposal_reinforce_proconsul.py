...
from rorapp.models import Campaign, Faction, Fleet, Game, Legion, Log, War


class ProposalReinforceProconsulEffect(EffectBase):
    ...
    def validate(self, game_state: GameStateSnapshot) -> bool:
        return (
            ...
            and game_state.game.current_proposal.startswith("Reinforce ")
        )
    ...
    def execute(self, game_id: int, random_resolver: RandomResolver) -> bool:
        ...
        if game.votes_yea > game.votes_nay:
            ...
            campaign_name_and_more = game.current_proposal[
                : game.current_proposal.find(" with")
            ]
            campaign = next(
                (
                    c
            ...
            # Parse legions and fleets from the proposal
            legion_pattern = (
            ...
            # Add the legions and fleets to the campaign
            ...
            # Mark campaign as recently reinforced
            campaign.recently_reinforced = True
            campaign.save()

            # Create log
            log_text = f"{unit_list_to_string(legions, fleets)}"
            if len(legions) + len(fleets) > 1:
                log_text += " were"
            else:
                log_text += " was"
            log_text += f" sent to reinforce {campaign.display_name} in the {war.name}."
