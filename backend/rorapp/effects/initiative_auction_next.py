...
class InitiativeAuctionNextEffect(EffectBase):
    ...
    def execute(self, game_id: int) -> bool:
        ...
        if next_faction.get_bid_amount() is not None:
            # Bid has been all the way around without being raised so the next faction has won the auction
            next_faction.add_status_item(Faction.StatusItem.AUCTION_WINNER)
            next_faction.save()
            for initiative_index in Faction.INITIATIVE_INDICES:
                if not any(
                    ...
                    Log.create_object(
                        game_id,
                        f"Sold to the highest bidder! {next_faction.display_name} won the auction for initiative {initiative_index}.",
                    )
                    break

        elif all(f.has_status_item(Faction.StatusItem.SKIPPED) for f in factions):
            # All factions skipped without bidding, so the HRAO gets the next initiative for free
            hrao_faction = None
            for faction in Faction.objects.filter(game=game_id):
                if any(
                    ...
                    hrao_faction = faction
                    break
            if not hrao_faction:
                return False

            hrao_faction.add_status_item(Faction.StatusItem.AUCTION_WINNER)
            hrao_faction.save()
            ...
            for initiative_index in Faction.INITIATIVE_INDICES:
                if not any(
                    ...
                    Log.create_object(
                        game_id,
                        f"{hrao_faction.display_name} gets initiative {initiative_index} for free because nobody bid in the auction.",
                    )
                    break
        ...
        for faction in Faction.objects.filter(game=game_id):
            ...
            if faction.has_status_item(Faction.StatusItem.SKIPPED):
                faction.remove_status_item(Faction.StatusItem.SKIPPED)
