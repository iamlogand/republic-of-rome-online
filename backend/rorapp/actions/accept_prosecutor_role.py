...
class AcceptProsecutorRoleAction(ActionBase):
    NAME = "Accept prosecutor role"
    POSITION = 0
    ...
    def execute(
        ...
        prosecutor = next(
            (s for s in senators if s.has_status_item(Senator.StatusItem.CONSENT_REQUIRED)),
            None,
        )
        if not prosecutor:
        ...
        prosecutor.add_status_item(Senator.StatusItem.PROSECUTOR)
        prosecutor.save()

        Log.create_object(
            game_id,
            f"{prosecutor.display_name} accepted the role of prosecutor.",
        )
