...
def get_minor_prosecution_reasons(
    senator: Senator, defeated_proposals: List[str]
) -> List[str]:
    ...
    for concession in senator.get_corrupt_concessions():
        formatted = f"corruption via {concession.value} concession"
        if formatted not in reasons:
            reasons.append(formatted)
    prefix = f"Prosecute {senator.display_name} for "
    failed_reasons = {
        p[len(prefix) :] for p in defeated_proposals if p.startswith(prefix)
    }
    return [r for r in reasons if r not in failed_reasons]


def has_minor_prosecution_target(
    senators: List[Senator], defeated_proposals: List[str], censor_id: int
) -> bool:
...
def has_major_prosecution_target(
    senators: List[Senator], defeated_proposals: List[str], censor_id: int
) -> bool:
    ...
    for senator in senators:
        ...
        if senator.has_status_item(Senator.StatusItem.MAJOR_CORRUPT):
            proposal = (
                f"Prosecute {senator.display_name} for major corruption in office"
            )
