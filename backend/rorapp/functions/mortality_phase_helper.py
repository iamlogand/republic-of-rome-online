...
def face_mortality(game, faction, action, step):
    ...
    completed_action = Action(
        step=step, faction=faction, type="face_mortality", required=True, completed=True
    )
    ...
    if Action.objects.filter(step__id=step.id, completed=False).count() == 0:
        ...
        for code in drawn_codes:
            senators = Senator.objects.filter(
                game=game, death_step__isnull=True, code=code
            )
            if senators.exists():
        ...
        if killed_senator_count == 0:
            ...
            action_log = ActionLog(
                index=new_action_log_index, step=step, type="face_mortality"
            )
            action_log.save()
            messages_to_send.append(
                create_websocket_message(
        ...
        new_phase = Phase(
            name="Forum", index=step.phase.index + 1, turn=step.phase.turn
        )
        new_phase.save()
        messages_to_send.append(
        ...
        first_faction = (
            Faction.objects.filter(game__id=game.id).order_by("rank").first()
        )
        action = Action(
            step=new_step,
