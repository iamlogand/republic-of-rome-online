...
All action classes are registered by name in `actions/meta/registry.py`. Actions can be controlled by feature flags — see [`docs/feature-flags.md`](feature-flags.md).
...
Effects are listed in priority order in `effects/meta/registry.py`. High-priority entries (game-over conditions) come first. Effects can be controlled by feature flags — see [`docs/feature-flags.md`](feature-flags.md).
